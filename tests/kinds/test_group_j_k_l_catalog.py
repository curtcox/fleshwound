...
class TestConvention:
    ...
    def test_conversation_carries_all_state_in_input_turns(self):
        value = assert_ok(
            run_step(
                ...
                provider=provider('{"reply": "Hello", "turns": [{"role": "user", "content": "Hi"}, {"role": "assistant", "content": "Hello"}]}'),
                kind="conversation",
            )
        )

        assert value["reply"] == "Hello"
        assert value["turns"] == [{"role": "user", "content": "Hi"}, {"role": "assistant", "content": "Hello"}]

    def test_tournament_uses_pairwise_judge_in_bracket_order(self):
        value = assert_ok(
            run_step(
                {"candidates": ["a", "b", "c"], "judge_kind": "pairwise_preference"},
                provider=provider('{"winner": "b", "rationale": "second wins", "confidence": 0.9}'),
                kind="tournament",
        ...
        assert [(match["a"], match["b"]) for match in value["bracket"]] == [("a", "b"), ("b", "c")]

    def test_pipeline_transformer_precondition_and_catalog_kinds(self):
        pipeline = assert_ok(
            run_step(
                {"initial": {"value": "x"}, "stages": [{"kind": "constant"}, {"kind": "echo"}]},
                kind="pipeline",
        ...
        assert {"name": "catalog_self_test", "convention": "run minimal inputs for listed kinds"} in listed["kinds"]

    def test_kind_chooser_prompt_is_grounded_in_catalog(self):
        fake = RecordingProvider('{"chosen_kind": "echo", "rationale": "echo returns input"}')

        value = assert_ok(run_step({"task": "return input unchanged"}, provider=fake, kind="kind_chooser"))

        assert value == {"chosen_kind": "echo", "rationale": "echo returns input"}
        assert '"echo": "input -> input verbatim; host charges one step"' in fake.prompts[0]
    ...
    def test_catalog_self_test_reports_unexpected_host_errors(self):
        ...
        assert {row["kind"]: row["expected_host_error"] for row in value["results"]}["noop_fail"] is True
    ...
    def test_catalog_self_test_exercises_registered_catalog(self):
        value = assert_ok(
            run_step(
                ...
                kind="catalog_self_test",
                budget={"tokens": 1_000_000, "steps": 300, "depth": 8, "tool_calls": 100},
            )
        )

        exercised = {row["kind"] for row in value["results"]}
        assert "catalog_self_test" not in exercised
        assert {"constant", "refine_until", "pipeline", "kind_chooser"}.issubset(exercised)
        assert value["unexpected_host_errors"] == []
...
class TestBudget:
    def test_refine_until_allocates_candidate_and_judge_each_round(self):
        ...
        assert [event.resolved_kind for event in ledger.events if event.kind == "allocate_child"] == [
            "constant",
            "judge",
            "constant",
            "judge",
...
class TestFailure:
    def test_pipeline_keeps_prior_value_when_stage_fails(self):
        value = assert_ok(
            run_step(
                {"initial": "start", "stages": [{"kind": "noop_fail"}, {"kind": "echo"}]},
                kind="pipeline",
                budget={"tokens": 0, "steps": 5, "depth": 3, "tool_calls": 0},
            )
        )
    ...
    def test_transformer_surfaces_bad_preprocess_as_executor_error(self):
        result = run_step(
            {"inner_input_template": {"value": "x"}, "preprocess": "input['missing']", "inner_kind": "constant"},
            kind="transformer",
        )
