...
class TestConvention:
    def test_map_reduce_maps_items_and_reduces_values(self):
        value = assert_ok(
            run_step(
                {"items": [{"value": 1}, {"value": 2}], "map_kind": "constant", "reduce_kind": "echo"},
                provider=provider(),
                kind="map_reduce",
            )
        )
    ...
    def test_retry_wrapper_stops_after_first_ok(self):
        value = assert_ok(
            run_step(
                {"inner_kind": "constant", "inner_input": {"value": "done"}, "max_attempts": 3},
                provider=provider(),
                kind="retry_wrapper",
            )
        )
    ...
    def test_ensemble_aggregates_with_model_when_prompt_is_supplied(self):
        fake = FakeProvider({r"Pick best": text_result("chosen-by-model", 2, 2)})
        ...
        assert value == {"chosen": "chosen-by-model", "candidates": ["candidate", "candidate"]}
    ...
    def test_clarify_then_delegate_records_question_answer_and_child_result(self):
        ...
        assert value["result"]["value"] == {"task": "summarize", "clarification": "make it short"}
...
class TestBudget:
    def test_map_reduce_allocates_children_in_order_and_refunds_each_child(self):
        ...
        child_ids = [event.budget_id for event in ledger.events if event.kind == "close_child"]
        assert child_ids == ["root.1", "root.2"]
        for child_id in child_ids:
            close_index = next(idx for idx, event in enumerate(ledger.events) if event.kind == "close_child" and event.budget_id == child_id)
    ...
    def test_retry_wrapper_refunds_failed_child_before_next_attempt(self):
        ...
        refund_indices = [idx for idx, event in enumerate(ledger.events) if event.kind == "refund_child"]
        close_indices = [idx for idx, event in enumerate(ledger.events) if event.kind == "close_child"]
        assert refund_indices
        assert all(refund < close for refund, close in zip(refund_indices, close_indices))
...
class TestFailure:
    ...
    def test_retry_wrapper_zero_attempts_returns_no_result(self):
        value = assert_ok(
            run_step(
                {"inner_kind": "constant", "inner_input": {"value": "unused"}, "max_attempts": 0},
                provider=provider(),
                kind="retry_wrapper",
            )
        )
    ...
    def test_judge_defaults_to_fail_when_model_does_not_pass(self):
        value = assert_ok(
            run_step(
                ...
                provider=provider("not good enough"),
                kind="judge",
            )
        )
...
class TestEndToEnd:
    def test_judge_passes_on_model_pass_text(self):
        value = assert_ok(
            run_step(
                {"candidate": "answer", "criteria": "strict"},
                provider=provider("PASS: meets the bar"),
                kind="judge",
            )
        )
...
class TestDeterminism:
    def test_map_reduce_is_deterministic(self):
        digest = assert_deterministic(
            "map_reduce",
            {"items": [{"value": "a"}, {"value": "b"}], "map_kind": "constant", "reduce_kind": "echo"},
            provider=provider(),
        )
    ...
    def test_ensemble_is_deterministic_with_frozen_provider(self):
        digest = assert_deterministic(
            "ensemble",
            {"inner_kind": "constant", "inner_input": {"value": "x"}, "n": 2, "aggregator_prompt": "Pick"},
            provider=provider("x"),
        )
