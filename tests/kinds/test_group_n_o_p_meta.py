...
class TestConvention:
    def test_scoring_and_grading_kinds_return_documented_shapes(self):
        rubric = assert_ok(
            run_step(
                {"candidate": "answer", "rubric": [{"criterion": "correctness", "weight": 2, "scale": "0-5"}]},
                provider=provider('{"scores": [{"criterion": "correctness", "score": 4, "rationale": "solid"}], "weighted_total": 8, "notes": "ok"}'),
                kind="rubric_grader",
            )
        )
        preference = assert_ok(
            run_step(
                ...
                provider=provider('{"winner": "b", "rationale": "clearer", "confidence": 0.8}'),
                kind="pairwise_preference",
            )
        )
        aggregate = assert_ok(
            run_step(
                {"scores": [{"score": 2, "weight": 1}, {"score": 4, "weight": 3}], "policy": "weighted_mean"},
    ...
    def test_adversarial_and_failure_kinds_round_trip_structured_values(self):
        attack = assert_ok(
            run_step(
                {"target_kind": "echo", "target_input_template": {"x": 1}, "attack_goal": "make it fail"},
                provider=provider('{"crafted_input": {"x": "bad"}, "rationale": "stress input"}'),
                kind="attack_generator",
            )
        )
        classified = assert_ok(
            run_step(
                {"step_result": {"outcome": "host_error", "value": None, "host_error": {"code": "executor_error", "message": "boom"}}},
                provider=provider('{"category": "host_error", "subcategory": "executor_error", "evidence": "boom"}'),
                kind="failure_classifier",
            )
        )
        canary = assert_ok(
            run_step(
                {"frozen_kind": "constant", "frozen_input": {"value": {"stable": True}}, "expected_value_hash": value_hash({"stable": True})},
    ...
    def test_translation_and_memo_kinds_compose_explicitly(self):
        adapted = assert_ok(
            run_step(
                {"source_kind": "echo", "target_kind": "constant", "source_value": {"value": "ready"}},
                provider=provider('{"target_input": {"value": "ready"}, "lossy": false, "notes": ""}'),
                kind="convention_adapter",
            )
        )
        chained = assert_ok(
            run_step(
                {"first_kind": "constant", "first_input": {"value": {"value": "ready"}}, "second_kind": "constant"},
                provider=provider('{"target_input": {"value": "ready"}, "lossy": false, "notes": ""}'),
        ...
        memo = assert_ok(
            run_step(
                {"inner_kind": "constant", "inner_input": {"value": "expensive"}, "memo": {}},
                kind="content_hash_memo",
                budget={"tokens": 0, "steps": 3, "depth": 2, "tool_calls": 0},
            )
        )

        assert adapted == {"target_input": {"value": "ready"}, "lossy": False, "notes": ""}
...
class TestBudget:
    def test_calibration_fans_out_once_per_example(self):
            ...
            run_step(
                {
                    ...
                    "examples": [{"item": "a", "gold_score": 1}, {"item": "b", "gold_score": 1}],
                },
                provider=provider('{"scores": [], "weighted_total": 1, "notes": ""}'),
                kind="calibration",
                ledger=ledger,
            )
        )
        ...
        assert [event.resolved_kind for event in ledger.events if event.kind == "allocate_child"] == ["rubric_grader", "rubric_grader"]
    ...
    def test_dedup_then_map_allocates_once_per_unique_item_hash(self):
        ...
        value = assert_ok(
            run_step(
                {"items": [{"value": "same"}, {"value": "same"}, {"value": "other"}], "inner_kind": "constant"},
                kind="dedup_then_map",
                ledger=ledger,
            )
        )
        ...
        assert [event.resolved_kind for event in ledger.events if event.kind == "allocate_child"] == ["constant", "constant"]
...
class TestFailure:
    def test_regression_canary_reports_hash_mismatch_as_value_not_host_error(self):
        value = assert_ok(
            run_step(
                {"frozen_kind": "constant", "frozen_input": {"value": "changed"}, "expected_value_hash": "not-the-hash"},
                kind="regression_canary",
                budget={"tokens": 0, "steps": 3, "depth": 2, "tool_calls": 0},
            )
        )
    ...
    def test_adversarial_loop_records_target_host_error_as_success(self):
        value = assert_ok(
            run_step(
                {"target_kind": "noop_fail", "seed_input": {}, "max_rounds": 1, "success_predicate": "host_error"},
                provider=provider('{"crafted_input": {}, "rationale": "force failure"}'),
        ...
        assert value["history"][0]["target_result"]["host_error"]["code"] == "executor_error"
    ...
    def test_content_hash_memo_hit_does_not_allocate_child(self):
        ...
        value = assert_ok(
            run_step(
                {"inner_kind": "constant", "inner_input": inner, "memo": {key: "cached"}},
                kind="content_hash_memo",
                ledger=ledger,
            )
        )
        ...
        assert [event for event in ledger.events if event.kind == "allocate_child"] == []
...
class TestDeterminism:
    def test_score_aggregator_is_deterministic(self):
        digest = assert_deterministic(
            "score_aggregator",
            {"scores": [{"score": 1, "weight": 1}, {"score": 3, "weight": 1}], "policy": "median"},
            provider=provider(),
    ...
    def test_regression_canary_is_deterministic(self):
        digest = assert_deterministic(
            "regression_canary",
            {"frozen_kind": "constant", "frozen_input": {"value": 1}, "expected_value_hash": value_hash(1)},
            provider=provider(),
            budget={"tokens": 0, "steps": 3, "depth": 2, "tool_calls": 0},
        )
