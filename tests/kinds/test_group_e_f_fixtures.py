...
class TestConvention:
    def test_always_partial_returns_deliberate_partial_value(self):
        value = assert_ok(run_step({}, provider=provider(), kind="always_partial"))

        assert value == {"status": "partial", "program": "", "notes": "deliberate partial for tests"}
...
class TestBudget:
    ...
    def test_budget_hog_tool_calls_records_denial_when_exhausted(self):
        ...
        value = assert_ok(run_step({"target": "tool_calls"}, provider=provider(), kind="budget_hog", ledger=ledger))

        assert value["target"] == "tool_calls"
        assert any(event.kind == "deny" and event.amount == {"tool_calls": 1} for event in ledger.events)
...
class TestFailure:
    def test_always_host_error_can_produce_every_contract_host_error_code(self):
        ...
        for code in sorted(HOST_ERROR_CODES):
            error = assert_host_error(run_step({"code": code}, provider=provider(), kind="always_host_error"), code)
