...
def _provider(*texts: str) -> FakeProvider:
    return FakeProvider({rf"iteration {idx}": text_result(text) for idx, text in enumerate(texts, start=1)})
...
def test_rlm_loop_delegates_to_child_step():
    ...
    value = assert_ok(
        run_step(
            {"task": "delegate", "max_iterations": 4, "child_request": {"tokens": 20, "steps": 2, "depth": 1, "tool_calls": 0}},
...
def test_rlm_loop_handles_unknown_child_kind_without_crashing():
    ...
    assert value["trace"][0]["observation"]["result"]["host_error"]["code"] == "unknown_kind"
...
def test_rlm_loop_handles_malformed_model_output():
    ...
    value = assert_ok(run_step({"task": "bad", "max_iterations": 1}, provider=fake, kind="rlm_loop"))
