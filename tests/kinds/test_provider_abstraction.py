...
class TestConvention:
    def test_llm_success_returns_always_dict_shape(self):
        ...
        provider = CallableProvider(lambda prompt: ModelTextResult("hello", Usage(2, 3)))

        value = assert_ok(
            run_step(
                {"prompt": "say hello"},
                {"tokens": 10, "steps": 1, "depth": 1, "tool_calls": 0},
                provider=provider,
                kind="llm_probe",
                catalog=catalog,
            )
...
class TestBudget:
    def test_llm_success_charges_reported_total_tokens(self):
        ...
        run_step({"prompt": "say hello"}, provider=provider, kind="llm_probe", catalog=catalog, ledger=ledger)
    ...
    def test_multiple_llm_calls_accumulate_charges(self):
        ...
        provider = CallableProvider(lambda prompt: ModelTextResult("hello", Usage(1, 2)))

        value = assert_ok(run_step({"prompt": "twice"}, provider=provider, kind="llm_then_value", catalog=catalog, ledger=ledger))

        assert value == {"first": "ok", "second": "ok", "done": True}
        assert [event.amount for event in ledger.events if event.kind == "charge_tokens"] == [{"tokens": 3}, {"tokens": 3}]
...
class TestFailure:
    def test_provider_exception_is_data_and_step_can_still_succeed(self):
        ...
        assert value["text"] == ""
        assert value["usage"] == {"prompt_tokens": 3, "completion_tokens": 0, "total_tokens": 3}
    ...
    def test_token_exhaustion_inside_llm_is_data_not_host_error(self):
        catalog = register_llm_probe()
        provider = CallableProvider(lambda prompt: ModelTextResult("too much", Usage(4, 4)))

        value = assert_ok(
            run_step(
                {"prompt": "overspend"},
                {"tokens": 3, "steps": 1, "depth": 1, "tool_calls": 0},
                provider=provider,
                kind="llm_probe",
                catalog=catalog,
            )
...
class TestDeterminism:
    def test_provider_results_are_deterministic_for_same_provider_and_input(self):
        catalog = register_llm_probe()
        provider = CallableProvider(lambda prompt: ModelTextResult(prompt.upper(), Usage(1, 1)))
        budget = {"tokens": 10, "steps": 1, "depth": 1, "tool_calls": 0}

        first = run_step({"prompt": "stable"}, budget, provider=provider, kind="llm_probe", catalog=catalog)
        second = run_step({"prompt": "stable"}, budget, provider=provider, kind="llm_probe", catalog=catalog)
