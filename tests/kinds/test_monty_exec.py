...
def provider(text: str = "model text", usage: Usage | None = None) -> CallableProvider:
    return CallableProvider(
        lambda prompt: ModelTextResult(
            text, usage or Usage(prompt_tokens=1, completion_tokens=1)
        )
    )
    ...
    def test_code_can_call_llm_external(self):
        value = assert_ok(
            run_step(
                ...
                kind="monty_exec",
                options=RunOptions(
                    budget={"tokens": 5, "steps": 1, "depth": 1, "tool_calls": 0},
    ...
    def test_monty_exec_charges_step_and_external_llm_tokens(self):
        ...
        value = assert_ok(
            run_step(
                ...
                kind="monty_exec",
                options=RunOptions(provider=provider("ok", Usage(2, 3)), ledger=ledger),
            )
        )
        assert value == "ok"
