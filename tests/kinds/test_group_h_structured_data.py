...
class TestConvention:
    def test_function_map_writer_returns_sources_keyed_by_signature_names(self):
        value = assert_ok(
            run_step(
                {
                    "signatures": {
                        "alpha": {"signature": "alpha() -> int", "docstring": "Return one."},
                        "beta": {"signature": "beta() -> int", "docstring": "Return one."},
                    },
                    "context": {"module": "demo"},
                },
    ...
    def test_function_map_editor_tracks_added_removed_and_updated_entries(self):
        value = assert_ok(
            run_step(
                {
                    "current": {"old": {"source": "keep"}, "gone": {"source": "remove"}},
                    "edits": [
        ...
        assert value["updated"] == {"old": {"source": "updated"}, "new": {"source": "added"}}
    ...
    def test_schema_diff_and_patch_set_parse_structured_model_json(self):
        schema = assert_ok(
            run_step(
                ...
                provider=provider('{"schema": {"type": "object"}, "rationale": "small"}'),
    ...
    def test_ast_transform_round_trips_nested_ast_data(self):
        ast = {"type": "Module", "body": [{"type": "Expr", "value": {"type": "Name", "id": "x"}}]}

        value = assert_ok(run_step({"ast": ast, "transform": "leave unchanged"}, provider=provider(), kind="ast_transform"))
...
class TestBudget:
    def test_function_map_writer_fans_out_once_per_signature_and_refunds_children(self):
        ...
        assert_ok(
            run_step(
                ...
                provider=provider(PROGRAM_RESPONSE, Usage(4, 4)),
                kind="function_map_writer",
                ledger=ledger,
            )
        )

        close_ids = [event.budget_id for event in ledger.events if event.kind == "close_child"]
        resolved = [event.resolved_kind for event in ledger.events if event.kind == "allocate_child"]
    ...
    def test_schema_designer_charges_model_usage(self):
        ...
        assert_ok(run_step({"domain": "todo"}, provider=provider('{"schema": {}}', Usage(3, 4)), kind="schema_designer", ledger=ledger))

        assert any(event.kind == "charge_tokens" and event.amount == {"tokens": 7} for event in ledger.events)
...
class TestFailure:
    ...
    def test_schema_designer_falls_back_when_model_returns_non_json(self):
        value = assert_ok(run_step({"domain": "todo"}, provider=provider("plain rationale"), kind="schema_designer"))

        assert value == {"schema": {}, "rationale": "plain rationale"}

    def test_patch_set_writer_falls_back_to_empty_patch_list_on_non_json(self):
        value = assert_ok(run_step({"files": {"a.txt": "old"}, "task": "change"}, provider=provider("not json"), kind="patch_set_writer"))
...
class TestDeterminism:
    def test_function_map_editor_is_deterministic(self):
        digest = assert_deterministic(
            "function_map_editor",
            {"current": {"a": {"source": "old"}}, "edits": [{"name": "a", "instruction": "new"}]},
            provider=provider(),
        )
