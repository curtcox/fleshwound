...
class TestConvention:
    def test_dynamic_dispatch_uses_literal_runtime_kind(self):
        value = assert_ok(
            run_step(
                {"chooser": "literal", "literal_kind": "constant", "inner_input": {"value": "chosen"}},
                provider=provider(),
    ...
    def test_dynamic_dispatch_can_choose_kind_with_llm_and_catalog_prompt(self):
        fake = FakeProvider({r"Choose one kind": text_result("echo", 1, 1)})

        value = assert_ok(
            run_step(
                {"chooser": "llm", "literal_kind": None, "task_for_chooser": "repeat input", "inner_input": {"x": 1}},
                provider=fake,
                kind="dynamic_dispatch",
            )
        )
    ...
    def test_meta_planner_executes_json_plan_from_model(self):
        fake = FakeProvider({r"Plan task with catalog": text_result('[{"kind": "constant", "input": {"value": 3}}]', 2, 2)})

        value = assert_ok(run_step({"task": "produce three"}, provider=fake, kind="meta_planner"))
...
class TestBudget:
    def test_runtime_chosen_kind_is_recorded_on_allocate_event(self):
        ...
        assert_ok(
            run_step(
                {"chooser": "literal", "literal_kind": "constant", "inner_input": {"value": "x"}},
                provider=provider(),
                kind="dynamic_dispatch",
                ledger=ledger,
            )
    ...
    def test_meta_planner_allocates_plan_children_in_order(self):
        ...
        fake = FakeProvider({r"Plan task with catalog": text_result('[{"kind": "echo", "input": 1}, {"kind": "constant", "input": {"value": 2}}]', 2, 2)})

        assert_ok(run_step({"task": "two steps"}, provider=fake, kind="meta_planner", ledger=ledger))
...
class TestFailure:
    def test_dynamic_dispatch_surfaces_unknown_runtime_kind_as_child_error(self):
        value = assert_ok(
            run_step(
                {"chooser": "literal", "literal_kind": "missing_kind", "inner_input": {}},
                provider=provider(),
                kind="dynamic_dispatch",
            )
        )
    ...
    def test_meta_planner_treats_malformed_model_plan_as_empty_plan(self):
        value = assert_ok(run_step({"task": "bad plan"}, provider=provider("not json"), kind="meta_planner"))
